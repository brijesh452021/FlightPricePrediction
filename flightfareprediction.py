# -*- coding: utf-8 -*-
"""FlightFarePrediction

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1XI5O7xUxHTloktM8l-2qVld8EpFGpwLj
"""

import pandas as pd
df=pd.read_excel("Data_Train.xlsx")
df.head()

df.shape

df.isnull().sum()

df.describe

df['Airline'].unique()

df['Airline'].value_counts()

import seaborn as sns
x=sns.FacetGrid(df,size=10)
x=x.map(sns.barplot,y=df['Airline'],x=df['Price'])

Airline=df[['Airline']]
Airline.head()

Airline=pd.get_dummies(Airline,drop_first=True)
Airline.head()

df['Source'].value_counts()

df['Source'].unique()

sns.FacetGrid(df,size=10).map(sns.barplot,y=df['Source'],x=df['Price'])

Source=df[['Source']]
Source.head()

Source=pd.get_dummies(Source,drop_first=True)
Source.head()

Destination=df[['Destination']]
Destination.head()

sns.FacetGrid(df,size=10).map(sns.barplot,y=df['Destination'],x=df['Price'])

df['Destination'].value_counts()

Destination=pd.get_dummies(Destination,drop_first=True)
Destination.head()

df.head()

df=pd.concat([df,Destination,Source,Airline],axis=1)
df.head()

df['Day_of_Journey']=pd.to_datetime(df['Date_of_Journey'],format="%d/%m/%Y").dt.day
df['Month_of_Journey']=pd.to_datetime(df['Date_of_Journey'],format='%d/%m/%Y').dt.month
df['Arrival_Hour']=pd.to_datetime(df['Arrival_Time']).dt.hour
df['Arrival_Min']=pd.to_datetime(df['Arrival_Time']).dt.minute
df['Dep_Hour']=pd.to_datetime(df['Dep_Time']).dt.hour
df['Dep_Min']=pd.to_datetime(df['Dep_Time']).dt.minute

df['Total_Stops'].value_counts()

def convertwordstoptoint(stops):
  Number_of_stops={'non-stop':0,'1 stop':1,'2 stops':2,'3 stops':3,'4 stops':4}
  return Number_of_stops[stops]

df["Total_Stops"].isnull().sum()
df["Total_Stops"].fillna('non-stop',inplace=True)
df["Total_Stops"].isnull().sum()

df["Total_Stops"]=df["Total_Stops"].apply(lambda x: convertwordstoptoint(x))
df['Total_Stops'].head()

df['Duration'].head()

def fetch_Duration_hrs(Duration):
  if 'h' in Duration:
    Duration=Duration.split(sep = "h")[0]
    return Duration


def fetch_Duration_mins(Duration):
  if 'm' in Duration:
    Duration=Duration.split(sep="m")[0].split()[-1]
    return Duration

df['Duration_Hrs']=df['Duration'].apply(lambda x:fetch_Duration_hrs(x))
df['Duration_mins']=df['Duration'].apply(lambda x:fetch_Duration_mins(x))

df.head()

df.drop(['Airline','Date_of_Journey','Source','Destination','Route','Dep_Time','Arrival_Time','Duration','Additional_Info'], axis=1,inplace=True)

df.head()

df.isnull().sum()

df.fillna(0, inplace=True)

df.head()

df.columns

X=df[['Total_Stops', 'Destination_Cochin', 'Destination_Delhi',
       'Destination_Hyderabad', 'Destination_Kolkata', 'Destination_New Delhi',
       'Source_Chennai', 'Source_Delhi', 'Source_Kolkata', 'Source_Mumbai',
       'Airline_Air India', 'Airline_GoAir', 'Airline_IndiGo',
       'Airline_Jet Airways', 'Airline_Jet Airways Business',
       'Airline_Multiple carriers',
       'Airline_Multiple carriers Premium economy', 'Airline_SpiceJet',
       'Airline_Trujet', 'Airline_Vistara', 'Airline_Vistara Premium economy',
       'Day_of_Journey', 'Month_of_Journey', 'Arrival_Hour', 'Arrival_Min',
       'Dep_Hour', 'Dep_Min', 'Duration_Hrs', 'Duration_mins']]

X.head()

y=df.iloc[:,1]
y.head()

##Test Data

df_test=pd.read_excel('Test_set.xlsx')
df_test.head()

Airline_test=df_test[['Airline']]
Source_test=df_test[['Source']]
Destination_test=df_test[['Destination']]

Airline_test=pd.get_dummies(Airline_test,drop_first=True)
Source_test=pd.get_dummies(Source_test,drop_first=True)
Destination_test=pd.get_dummies(Destination_test,drop_first=True)
df_test=pd.concat([df_test,Destination_test,Source_test,Airline_test],axis=1)

df_test['Day_of_Journey']=pd.to_datetime(df_test['Date_of_Journey'],format="%d/%m/%Y").dt.day
df_test['Month_of_Journey']=pd.to_datetime(df_test['Date_of_Journey'],format='%d/%m/%Y').dt.month
df_test['Arrival_Hour']=pd.to_datetime(df_test['Arrival_Time']).dt.hour
df_test['Arrival_Min']=pd.to_datetime(df_test['Arrival_Time']).dt.minute
df_test['Dep_Hour']=pd.to_datetime(df_test['Dep_Time']).dt.hour
df_test['Dep_Min']=pd.to_datetime(df_test['Dep_Time']).dt.minute

def convertwordstoptoint(stops):
  Number_of_stops={'non-stop':0,'1 stop':1,'2 stops':2,'3 stops':3,'4 stops':4}
  return Number_of_stops[stops]
  
  
df_test["Total_Stops"].fillna('non-stop',inplace=True)
df_test["Total_Stops"].isnull().sum()

df_test["Total_Stops"]=df_test["Total_Stops"].apply(lambda x: convertwordstoptoint(x))

def fetch_Duration_hrs(Duration):
  if 'h' in Duration:
    Duration=Duration.split(sep = "h")[0]
    return Duration


def fetch_Duration_mins(Duration):
  if 'm' in Duration:
    Duration=Duration.split(sep="m")[0].split()[-1]
    return Duration

df_test['Duration_Hrs']=df_test['Duration'].apply(lambda x:fetch_Duration_hrs(x))
df_test['Duration_mins']=df_test['Duration'].apply(lambda x:fetch_Duration_mins(x))


df_test.fillna(0, inplace=True)

df_test.head()

df_test.drop(['Airline','Date_of_Journey','Source','Destination','Route','Dep_Time','Arrival_Time','Duration','Additional_Info'], axis=1,inplace=True)

import matplotlib.pyplot as plt
plt.figure(figsize=(20,20))
corrmat=df.corr()
topfeatures=corrmat.index
sns.heatmap(df[topfeatures].corr(),annot=True,cmap=None)
plt.show()

from sklearn.ensemble import ExtraTreesRegressor

extReg=ExtraTreesRegressor()
extReg.fit(X,y)
extReg.feature_importances_

plt.figure(figsize=(15,15))
Imp_feature=pd.Series(extReg.feature_importances_,index=X.columns )
Imp_feature.plot(kind="bar")
plt.show()

from sklearn.model_selection import train_test_split

train_x,test_x,train_y,test_y=train_test_split(X,y,test_size=0.3,random_state=0)

from sklearn.model_selection import RandomizedSearchCV
from sklearn.ensemble import RandomForestRegressor
rf=RandomForestRegressor()

import numpy as np
n_estimators=[int(x) for x in np.linspace(start=100,stop=1000,num=10)]
criterion=['mse','mae']
max_depth=[1,2,5,10,20,100]
min_samples_split=[1,2,5,10,20,100]
min_samples_leaf=[1,2,5,10,20,100]
max_features=['auto','sqrt']
max_samples=[1,2,5,10,20,100]

param_dist={'n_estimators':n_estimators,
            'criterion':criterion,
            'max_depth':max_depth,
            'min_samples_split':min_samples_split,
            'min_samples_leaf':min_samples_leaf,
            'max_features':max_features,
            'max_samples':max_samples
            }

rf_random=RandomizedSearchCV(estimator=rf,param_distributions=param_dist,cv=5,verbose=2)
rf_random.fit(train_x,train_y)

rf_random.best_estimator_

rf_random.best_params_

rf_random.best_score_

predt=rf_random.predict(test_x)

plt.figure(figsize=(15,15))
plt.scatter(test_y,predt)
plt.show()

plt.figure(figsize=(15,15))
sns.distplot(test_y-predt)

from sklearn import metrics
print('MAE:', metrics.mean_absolute_error(test_y, predt))
print('MSE:', metrics.mean_squared_error(test_y, predt))

import pickle

pickle.dump(rf_random,open('FlightFarePrediction.pkl','wb'))

FlightFarePredictionpickle=pickle.load(open('FlightFarePrediction.pkl','rb'))
predictions=FlightFarePredictionpickle.predict(test_x)
metrics.r2_score(test_y, predictions)